...
class Check_file:
    def __init__(self, file: str):
        ...
        self.mode = stat(file).st_mode

    def size_file(self) -> str:
        """
        !This function is responsible for calling a method to see the size of the file
        """
        return str(stat(self.file).st_size)
    ...
    def time(self) -> str:
        ...
        last_time = str(datetime.datetime.fromtimestamp(
            stat(self.file).st_mtime))[:19]

        return last_time

    def file_extencion(self) -> str:
        """
        ...
        if path.islink(self.file):
            return "Is  A Symbolic Link"
        elif S_ISDIR(self.mode):
            return "Directory"

        for exten, descip in file_type.items():
            if S_ISREG(self.mode):
                if access(self.file, X_OK):
                    return "Executable"
    ...
    def permisson(self):
        ...
        # Owner permissions
        permisson_file += "r" if self.mode & S_IRUSR else "-"
        permisson_file += "w" if self.mode & S_IWUSR else "-"
        permisson_file += "x" if self.mode & S_IXUSR else "-"

        # Group permissions
        permisson_file += "r" if self.mode & S_IRGRP else "-"
        permisson_file += "w" if self.mode & S_IWGRP else "-"
        permisson_file += "x" if self.mode & S_IXGRP else "-"

        # Others permissions
        permisson_file += "r" if self.mode & S_IROTH else "-"
        permisson_file += "w" if self.mode & S_IWOTH else "-"
        permisson_file += "x" if self.mode & S_IXOTH else "-"

        return permisson_file

    def run(self) -> None:
        size: str = self.size_file()
        time: str = self.time()
        type_file: str = self. file_extencion()
        permissions: str = self.permisson()

        print(f"File Name: {c.g}{self.file}{c.off}")
